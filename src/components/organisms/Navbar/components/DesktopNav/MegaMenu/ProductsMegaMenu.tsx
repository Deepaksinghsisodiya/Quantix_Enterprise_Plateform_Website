          ...
          <div className="col-span-4 border-r border-slate-200/80 dark:border-slate-800/80 pr-8 flex flex-col gap-4 justify-between">
            {promoCards.map((card, idx) => (
              <Link
                ...
                onClick={onClose}
                className="group/card relative flex items-center gap-4 p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-primary/40 hover:bg-slate-50/50 dark:hover:bg-slate-850 transition-all duration-300 shadow-2xs hover:shadow-md overflow-hidden"
              >
                ...
                <div className="flex-1 space-y-1 min-w-0">
                  <span className="inline-flex items-center gap-1 text-[9px] font-syne font-black uppercase tracking-wider text-primary-dark dark:text-primary-light bg-primary/10 dark:bg-primary/20 border border-primary/20 px-2 py-0.5 rounded-full">
                    <Sparkles size={9} />
                    {card.badge}
                  </span>
