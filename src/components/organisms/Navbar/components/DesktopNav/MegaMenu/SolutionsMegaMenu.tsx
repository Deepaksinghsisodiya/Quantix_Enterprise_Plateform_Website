        ...
        {promoCards && promoCards.length > 0 && (
          <div className="flex flex-col border-slate-200/80 pr-0 dark:border-slate-800/80 lg:col-span-4 lg:border-r lg:pr-7 h-full">
            {promoCards.map((card, idx) => {
              const isCardActive = pathname === card.href || (card.href !== '/' && pathname.startsWith(`${card.href}/`));
              ...
              return (
                <Link
                  ...
                  onClick={onClose}
                  className={cn(
                    "group/card relative flex flex-col justify-between h-full gap-3.5 p-4.5 rounded-2xl border transition-all duration-300 shadow-2xs hover:shadow-md overflow-hidden",
                    isCardActive
                      ? "bg-amber-500/10 dark:bg-amber-500/15 border-amber-500/40 shadow-xs"
                      : "bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 hover:border-amber-500/40 hover:bg-slate-50/70 dark:hover:bg-slate-850"
                  )}
                >
                  <div className="relative w-full h-44 sm:h-48 lg:h-46.25 rounded-xl overflow-hidden shrink-0 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xs flex items-center justify-center p-2.5">
                    ...
                    <div className="absolute top-2 left-2">
                      <span className="inline-flex items-center gap-1 text-[8.5px] font-syne font-black uppercase tracking-wider text-amber-700 dark:text-amber-400 bg-amber-100/90 dark:bg-amber-900/30 border border-amber-300/40 px-2 py-0.5 rounded-full">
                        <Sparkles size={8.5} />
                        {card.badge}
                      </span>
                    </div>
                  ...
                  <div className="space-y-1 min-w-0 pb-1">
                    <div className="text-[14px] font-syne font-bold text-slate-900 dark:text-white group-hover/card:text-amber-600 dark:group-hover/card:text-amber-400 transition-colors leading-snug">
