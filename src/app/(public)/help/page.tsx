  ...
  return (
    <PublicLayout>
      ...
      <main className="pt-24 bg-slate-950 min-h-screen text-white pb-16">
        ...
        {/* Quick Links Matrix */}
        <div className="site-container max-w-4xl grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
          {[
            ...
            <Link key={idx} href={link.href}>
              <div className="bg-slate-900/20 border border-slate-800 p-5 rounded-2xl space-y-2 hover:border-blue-500/30 transition-all cursor-pointer h-full">
                <div className="h-8 w-8 rounded-lg bg-blue-500/10 text-blue-400 flex items-center justify-center border border-blue-500/20">
                  {link.icon}
                </div>
                <h3 className="text-xs font-bold text-white uppercase font-syne">{link.title}</h3>
        ...
        <div className="site-container max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-6 mb-16">
          <div className="rounded-3xl border border-slate-800/80 bg-slate-900/40 p-6 sm:p-8 backdrop-blur-md relative overflow-hidden flex flex-col justify-between">
            <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-blue-500/40 to-transparent" />
            <div className="space-y-3">
              <h3 className="text-sm font-syne font-bold uppercase tracking-tight text-white">Standalone Setup & Token Management</h3>
            ...
            <Link href="/roi-calculator">
              <button className="text-xs font-bold text-blue-400 hover:text-blue-300 transition-all flex items-center gap-1 mt-6 cursor-pointer">
          ...
          <div className="rounded-3xl border border-slate-800/80 bg-slate-900/40 p-6 sm:p-8 backdrop-blur-md relative overflow-hidden flex flex-col justify-between">
            <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-blue-500/40 to-transparent" />
            <div className="space-y-3">
              <h3 className="text-sm font-syne font-bold uppercase tracking-tight text-white">Enterprise Cloud Configuration</h3>
              <p className="text-xs text-slate-400 leading-relaxed font-medium">
        ...
        <div className="site-container max-w-3xl space-y-6">
          <h2 className="text-xl sm:text-2xl font-syne font-black uppercase text-center tracking-tight text-white mb-6">Frequently Asked Questions</h2>

          {isLoading ? (
            <div className="text-center py-10 text-slate-500 text-xs">Loading support matrix...</div>
          ) : filteredFaqs.length > 0 ? (
            <div className="space-y-3">
              {filteredFaqs.map((faq) => (
                <div key={faq.id} className="rounded-2xl border border-slate-800/80 bg-slate-900/30 overflow-hidden transition-all">
                  <button
                    onClick={() => toggleFaq(faq.id)}
                    className="w-full text-left p-5 text-xs font-bold uppercase text-white hover:bg-slate-900/40 transition-all flex justify-between items-center cursor-pointer gap-4"
                  >
                  ...
                  <AnimatePresence>
                    {openFaqId === faq.id && (
                      ...
                      >
                        <div className="p-5 border-t border-slate-800/60 text-xs text-slate-400 leading-relaxed font-medium bg-slate-950/20">
                          {faq.answer}
                        </div>
                      </motion.div>
                    )}
            ...
            <div className="text-center py-10 text-slate-500 text-xs">No answers found for your query. Try different terms.</div>
          )}
        </div>
      </main>
      <Footer />
