...
export const IntegrationGrid: React.FC<IntegrationGridProps> = ({ integrations, isLoading }) => {
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="flex flex-col md:flex-row gap-6 md:items-center justify-between bg-slate-900/40 p-6 rounded-2xl border border-slate-800/80 backdrop-blur-md">
        <div className="relative max-w-md flex-1">
          ...
          <input
            ...
            className="w-full rounded-full border border-slate-700/50 bg-slate-950/60 py-3 pl-11 pr-5 text-sm text-slate-200 placeholder-slate-500 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/10 transition-all duration-200"
          />
        </div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {Array.from({ length: 6 }).map((_, idx) => (
            <div key={idx} className="h-44 rounded-2xl border border-slate-800/80 bg-slate-900/20 p-6 animate-pulse" />
          ))}
        </div>
      ) : filtered.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((integration) => (
            <motion.div
              ...
              key={integration.id}
              className="group flex flex-col justify-between rounded-2xl border border-slate-800 bg-slate-900/20 p-6 transition-all duration-300 hover:border-blue-500/50 hover:bg-slate-900/40 relative overflow-hidden"
            >
              ...
              <div>
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-slate-900/80 border border-slate-800/80 mb-5 group-hover:border-blue-500/30 transition-colors">
                  {integration.logoUrl ? (
                    <img src={integration.logoUrl} alt={integration.name} className="h-8 w-8 object-contain" />
                  ) : (
                    <span className="text-xl font-syne font-black text-slate-400 group-hover:text-blue-400 transition-colors">
                      {integration.name.charAt(0)}
                    </span>
                  )}
                </div>

                <h3 className="text-base font-syne font-bold text-white group-hover:text-blue-400 transition-colors">
                  {integration.name}
                </h3>
              ...
              <div className="mt-6 flex items-center justify-between border-t border-slate-800/60 pt-4">
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 bg-slate-900 px-2.5 py-1 rounded-md border border-slate-800/50 capitalize">
        ...
        <div className="py-16 text-center bg-slate-900/10 border border-slate-800/60 rounded-2xl">
      ...
      <div className="rounded-2xl border border-slate-800/80 bg-slate-900/20 p-8 sm:p-10 backdrop-blur-md relative overflow-hidden">
        ...
        <div className="max-w-xl mx-auto text-center space-y-4">
          <h3 className="text-xl sm:text-2xl font-syne font-black text-white uppercase tracking-tight">
          ...
          <AnimatePresence mode="wait">
            {!requestSubmitted ? (
              <form onSubmit={handleSubmitRequest} className="mt-8 space-y-4 text-left max-w-md mx-auto">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <input
                    ...
                    className="w-full rounded-xl border border-slate-800 bg-slate-950/60 px-4 py-3 text-xs text-white placeholder-slate-500 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                  />
                  <input
                    type="email"
                    required
              ...
              <motion.div
                ...
                className="mt-6 p-6 rounded-xl bg-blue-950/20 border border-blue-500/20 text-center space-y-2 max-w-sm mx-auto"
              >
                <div className="h-8 w-8 rounded-full bg-blue-600 flex items-center justify-center mx-auto mb-2 text-white">
                  <Check size={16} className="stroke-[3]" />
                </div>
                <h4 className="text-xs font-bold text-white uppercase tracking-wider">Request Logged</h4>
                <p className="text-[11px] text-slate-400 font-medium">We will update you at {requestEmail} as soon as this connector is ready!</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
