      ...
      {isLoading ? (
        <div className="space-y-4">
          {Array.from({ length: 3 }).map((_, idx) => (
            <div key={idx} className="h-40 rounded-2xl border border-slate-800 bg-slate-900/20 animate-pulse" />
          ))}
        </div>
      ) : (
        <div className="grid gap-6 grid-cols-1">
          {downloadPackages.map((pkg) => (
            <div
              key={pkg.packageId}
              className="rounded-2xl border border-slate-800 bg-slate-900/20 p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 transition-all duration-300 hover:border-blue-500/40 hover:bg-slate-900/40 relative overflow-hidden group"
            >
              {pkg.isLatest && (
                <div className="absolute top-0 right-0 bg-emerald-500/10 border-l border-b border-emerald-500/20 text-emerald-400 font-extrabold text-[9px] uppercase tracking-wider px-3.5 py-1 rounded-bl-xl flex items-center gap-1">
                  <ShieldCheck size={12} className="stroke-[3]" /> Stable Build
                </div>
              )}

              <div className="flex items-start gap-4">
                <div className="h-12 w-12 flex-shrink-0 flex items-center justify-center rounded-xl bg-slate-900/80 border border-slate-800/80 text-blue-400 group-hover:border-blue-500/30 transition-colors">
                ...
                <div className="space-y-1">
                  <div className="flex flex-wrap items-center gap-2.5">
                    <h3 className="text-base sm:text-lg font-syne font-bold text-white group-hover:text-blue-400 transition-colors">
                      {pkg.name}
                    </h3>
              ...
              <div className="w-full md:w-auto flex items-center justify-between md:justify-end gap-6 border-t md:border-0 border-slate-800/50 pt-4 md:pt-0">
