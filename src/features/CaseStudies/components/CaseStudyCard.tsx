...
export const CaseStudyCard: React.FC<CaseStudyCardProps> = ({ study, isFeatured = false }) => {
  return (
    <motion.div
      ...
      className={`group relative flex flex-col justify-between rounded-md bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border border-slate-200/90 dark:border-slate-800/90 shadow-2xs hover:shadow-md hover:border-primary/50 dark:hover:border-primary/50 transition-all duration-300 p-5 sm:p-6 overflow-hidden h-full ${
        isFeatured ? 'max-w-3xl mx-auto w-full' : ''
      }`}
    >
      {/* Top Accent Glowing Laser Line on Hover */}
      <div className="absolute top-0 left-0 right-0 h-[2px] bg-linear-to-r from-[#FF4F00] via-[#FF6B2B] to-amber-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      ...
      <div className="relative z-10">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2.5 mb-3.5">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="font-mono text-xs font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider truncate">
              {study.companyName}
            </span>
          </div>

          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[10px] font-mono font-bold uppercase tracking-wider bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200/80 dark:border-slate-700/80 shrink-0 shadow-2xs">
        ...
        <div className="my-3.5 p-3.5 sm:p-4 rounded-md bg-linear-to-br from-orange-500/8 via-amber-500/4 to-transparent border border-orange-500/25 dark:border-orange-500/30 flex items-center justify-between gap-3 shadow-2xs group-hover:border-orange-500/40 transition-colors">
          <div>
            <div className="text-2xl sm:text-3xl font-syne font-black text-transparent bg-clip-text bg-linear-to-r from-[#FF4F00] via-[#FF6B2B] to-amber-500 tracking-tight leading-none">
              {study.statValue || '+38%'}
            </div>
          ...
          <div className="h-9 w-9 sm:h-10 sm:w-10 rounded-md bg-orange-500/10 flex items-center justify-center text-[#FF4F00] shrink-0 border border-orange-500/20 group-hover:scale-105 transition-transform">
        ...
        <div className="space-y-2.5 mb-5">
          {/* Pain Point */}
          <div className="flex items-start gap-2.5 p-3 rounded-md bg-slate-50 dark:bg-slate-800/40 border border-slate-200/60 dark:border-slate-800/70">
          ...
          {/* Verified Outcome */}
          <div className="flex items-start gap-2.5 p-3 rounded-md bg-emerald-500/6 dark:bg-emerald-950/20 border border-emerald-500/20">
